...
import { StatusCode, StatusCodeRegistry } from "../dictionaries/http.ts";
import * as PublicInterfaces from "../interfaces.ts";
 ...
 */
export class DrashResponse implements PublicInterfaces.DrashResponse {
  #state: {
    body?: BodyInit | null;
    error?: Error;
    headers?: Headers;
  ...
  public error(statusCode: StatusCode, reason?: string): this {
  ...
  public redirect(
    ...
    this.status(StatusCode.TemporaryRedirect);
  ...
  #setResponseBody(contentType: string, body: BodyInit | null): this {
    // TODO(crookse) Should we valiate the content type against MimeDB?
    this.headers({
