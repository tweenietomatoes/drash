...
export type MIMEType = {
  ...
  [key: string]: {
    ...
    extensions?: string[];
    /** Where is the MIME type defined (e.g., Apache, IANA, Nginx). */
    source?: string;
  };
};
...
export type ErrorHandlerContext = {
  ...
  response: Interfaces.DrashResponse;
};
...
export type ContextForRequest = {
   ...
   * the context is created with only the `DrashRequest` and `DrashResponse`
   * objects. There is no resource yet until later in the lifecycle.
   */
  resource_handler?: Interfaces.ResourceHandler;

  /**
   * The `DrashResponse` instance, not the interface.
   */
  response: Interfaces.DrashResponse;
};
...
export type HTTPStatusCodeEnum = HTTPStatusCode;
